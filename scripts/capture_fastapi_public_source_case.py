...
def main() -> None:
    ...
    configured_python = str(os.environ.get("PYTHON_BIN") or os.environ.get("SANGUO_RAG_PYTHON") or "").strip()
    if configured_python:
        python_exe = Path(configured_python)
    else:
        default_python = repo_root / ".venv" / "Scripts" / "python.exe"
        python_exe = default_python if default_python.is_file() else Path(sys.executable)
