...
def render_report(payload: dict) -> str:
    ...
    for row in payload["rows"]:
        warnings = ", ".join([*row["qualityWarnings"], *row["resolutionWarnings"]]) or "-"
        lines.append(
            f"| `{row['generalId']}` | `{row['status']}` | `{row['persona']}` | "
            f"`{row['contextCount']}` | `{row['keywordCategoryCount']}` | "
            f"`{row['usedEvidenceRefCount']}` | `{row['fallbackUsed']}` | `{warnings}` |"
        )
    lines.append("")
    for row in payload["rows"]:
        lines.extend(
...
def main() -> None:
    ...
    general_ids = args.general_id or DEFAULT_GENERAL_IDS
