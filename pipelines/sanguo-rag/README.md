...
`pipelines/sanguo-rag/` 是三國大腦中台的正式 ETL / RAG pipeline 腳本位置。

以下範例假設已在目標 venv 內執行；若還沒，先 `source` 對應的 `bin/activate`，或設定 `PYTHON_BIN` 指向該環境。

API service 層位於 `app/`。Pipeline 產出 canonical fixtures，service 再把 fixtures 轉為 Cocos 可呼叫的 `/v1/npc/*` DTO；兩層不要互相混寫。
...
```bash
python pipelines/sanguo-rag/clean_and_split.py \
...
```bash
python pipelines/sanguo-rag/check_event_alias_hits.py --overwrite
```
...
```bash
python pipelines/sanguo-rag/resolve_dialogue_mentions.py --overwrite
```
...
```bash
python pipelines/sanguo-rag/extract_event_candidates.py --overwrite
```
...
```bash
python pipelines/sanguo-rag/build_keyword_options.py \
...
```bash
python pipelines/sanguo-rag/validate_llm_extraction_trial.py --overwrite
```
...
```bash
python pipelines/sanguo-rag/run_deepseek_reasoning_trial.py \
	--general-id zhang-fei \
...
```bash
python pipelines/sanguo-rag/run_etl_quality_pilot.py \
	--top 24 \
	--include-cold 4 \
	--overwrite
```
...
```bash
python pipelines/sanguo-rag/generate_event_review_choices.py \
	--general-id lu-bu \
...
```bash
python pipelines/sanguo-rag/enrich_event_review_context.py \
...
```bash
python pipelines/sanguo-rag/collect_observed_mentions.py \
...
```bash
python pipelines/sanguo-rag/run_resolution_loop.py --top 30
```
...
```bash
python pipelines/sanguo-rag/apply_triage_answers.py
python pipelines/sanguo-rag/run_resolution_loop.py --top 30
```
...
```bash
python pipelines/sanguo-rag/run_resolution_loop.py --top 30 --auto-fill-suggestions --apply-answers
```
...
```bash
python pipelines/sanguo-rag/generate_term_research_brief.py --top 30
```
...
- `pipelines/sanguo-rag/config/manual-roster-seeds.json`
