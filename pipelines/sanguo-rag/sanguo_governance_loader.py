...
def load_dialogue_mention_resolution_policy(
    root: str | Path | None = None,
    *,
    dialogue_mention_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(dialogue_mention_policy).resolve()
        if dialogue_mention_policy
        else _path(base, "policies", "policy-dialogue-mention-resolution.json")
    )
    return read_governance_json(path, required_id="Policy_DialogueMentionResolution_P1")


def load_dialogue_mention_resolution_cue_rules(
    root: str | Path | None = None,
    *,
    dialogue_mention_cue_rules: str | Path | None = None,
) -> list[dict[str, Any]]:
    base = resolve_governance_root(root)
    path = (
        Path(dialogue_mention_cue_rules).resolve()
        if dialogue_mention_cue_rules
        else _path(base, "rules", "rule-dialogue-mention-resolution-cues.jsonl")
    )
    return read_governance_jsonl(path, required_fields=("id", "consumer", "constantName", "kind", "value"))
...
def load_governance_release_readiness_policy(
    root: str | Path | None = None,
    *,
    governance_release_readiness_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_release_readiness_policy).resolve()
        if governance_release_readiness_policy
        else _path(base, "policies", "policy-governance-release-readiness.json")
    )
...
def load_governance_failure_triage_policy(
    root: str | Path | None = None,
    *,
    governance_failure_triage_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_failure_triage_policy).resolve()
        if governance_failure_triage_policy
        else _path(base, "policies", "policy-governance-failure-triage.json")
    )
    return read_governance_json(path, required_id="Policy_GovernanceFailureTriage_P1")


def load_governance_completion_ledger_policy(
    root: str | Path | None = None,
    *,
    governance_completion_ledger_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_completion_ledger_policy).resolve()
        if governance_completion_ledger_policy
        else _path(base, "policies", "policy-governance-completion-ledger.json")
    )
...
def load_governance_plan_encoding_repair_policy(
    root: str | Path | None = None,
    *,
    governance_plan_encoding_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_plan_encoding_policy).resolve()
        if governance_plan_encoding_policy
        else _path(base, "policies", "policy-governance-plan-encoding-repair.json")
    )
...
def load_governance_ci_entrypoint_policy(
    root: str | Path | None = None,
    *,
    governance_ci_entrypoint_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_ci_entrypoint_policy).resolve()
        if governance_ci_entrypoint_policy
        else _path(base, "policies", "policy-governance-ci-entrypoint.json")
    )
    return read_governance_json(path, required_id="Policy_GovernanceCiEntrypoint_P1")


def load_governance_runbook_policy(
    root: str | Path | None = None,
    *,
    governance_runbook_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(governance_runbook_policy).resolve()
        if governance_runbook_policy
        else _path(base, "policies", "policy-governance-runbook.json")
    )
...
def load_postgres_state_store_evaluation_policy(
    root: str | Path | None = None,
    *,
    postgres_state_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(postgres_state_policy).resolve()
        if postgres_state_policy
        else _path(base, "policies", "policy-postgres-state-store-evaluation.json")
    )
    return read_governance_json(path, required_id="Policy_PostgresStateStoreEvaluation_P1")


def load_vector_ingestion_hardening_policy(
    root: str | Path | None = None,
    *,
    vector_ingestion_hardening_policy: str | Path | None = None,
) -> dict[str, Any]:
    base = resolve_governance_root(root)
    path = (
        Path(vector_ingestion_hardening_policy).resolve()
        if vector_ingestion_hardening_policy
        else _path(base, "policies", "policy-vector-ingestion-hardening.json")
    )
