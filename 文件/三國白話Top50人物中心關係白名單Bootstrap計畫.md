...
## 4.5 `SANGUO-BOOTSTRAP-0001` 落地檔案映射

本任務卡完成後，資料契約與治理設定對應以下檔案：

1. lane policy：`data/sanguo/policies/policy-baihua-bootstrap-lane.json`
2. job schema：`data/sanguo/schemas/schema-baihua-bootstrap-job.json`
3. skill output schema：`data/sanguo/schemas/schema-baihua-bootstrap-skill-output.json`
4. merged candidate schema：`data/sanguo/schemas/schema-baihua-bootstrap-candidate.json`

以上四份檔案都要求 `canonicalWrites=false`，確保 bootstrap lane 只產候選，不直接改寫最終 whitelist / blacklist。
